...
vi.mock('primevue/inputnumber', () => ({
  default: {
    name: 'InputNumber',
    template: '<input type="number" v-model="modelValue" @update:model-value="$emit(\'update:model-value\', $event)" />',
...
vi.mock('primevue/slider', () => ({
  default: {
    name: 'Slider',
    template: '<input type="range" v-model="modelValue" @update:model-value="$emit(\'update:model-value\', $event)" />',
...
describe('ItemInterpolationBar', () => {
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle ranges without base_aoid', () => {
      const rangesWithoutBaseAoid = [
        { min_ql: 100, max_ql: 200 }
      ]
      wrapper = createWrapper({ 
