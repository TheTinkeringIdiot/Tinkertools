<template>
  ...
  >
    <div v-if="weapons.length === 0" class="text-center py-8">
      ...
      <p class="text-gray-600">No weapons selected for comparison</p>
    </div>
    ...
    <div v-else class="overflow-x-auto">
      <table class="min-w-full border-collapse border border-gray-200">
        ...
        <thead class="bg-gray-50">
          <tr>
            <th class="border border-gray-200 px-4 py-2 text-left">Property</th>
            <th 
              v-for="weapon in weapons" 
              :key="weapon.id"
              class="border border-gray-200 px-4 py-2 text-left relative"
            >
              <div class="flex items-start justify-between">
                <div class="flex-1 min-w-0">
                  <div class="font-medium truncate">{{ weapon.name }}</div>
                  <div class="text-xs text-gray-500">QL {{ weapon.ql }}</div>
                </div>
                <Button
        ...
        <tbody>
          <!-- Basic Properties -->
          <tr>
            <td class="border border-gray-200 px-4 py-2 font-medium bg-gray-50">Quality Level</td>
            <td 
              v-for="weapon in weapons" 
              :key="weapon.id"
          ...
          <tr>
            <td class="border border-gray-200 px-4 py-2 font-medium bg-gray-50">Item Class</td>
            <td 
              v-for="weapon in weapons" 
              :key="weapon.id"
              class="border border-gray-200 px-4 py-2"
          ...
          <tr v-if="characterSkills && Object.keys(characterSkills).length > 0">
            <td class="border border-gray-200 px-4 py-2 font-medium bg-gray-50">Usability</td>
            <td 
              v-for="weapon in weapons" 
              :key="weapon.id"
              class="border border-gray-200 px-4 py-2"
          ...
          <!-- Skill Requirements -->
          <tr>
            <td class="border border-gray-200 px-4 py-2 font-medium bg-gray-50">Requirements</td>
            <td 
              v-for="weapon in weapons" 
              :key="weapon.id"
              class="border border-gray-200 px-4 py-2"
          ...
          <tr v-if="hasAttackStats">
            <td class="border border-gray-200 px-4 py-2 font-medium bg-gray-50">Attack Stats</td>
            <td 
              v-for="weapon in weapons" 
              :key="weapon.id"
              class="border border-gray-200 px-4 py-2"
            >
              <div v-if="weapon.attack_stats && weapon.attack_stats.length > 0" class="space-y-1">
                ...
                <div v-if="weapon.attack_stats.length > 5" class="text-xs text-gray-500">
                  +{{ weapon.attack_stats.length - 5 }} more
                </div>
              </div>
              <div v-else class="text-xs text-gray-500">None</div>
            </td>
          </tr>

          <!-- Defense Stats -->
          <tr v-if="hasDefenseStats">
            <td class="border border-gray-200 px-4 py-2 font-medium bg-gray-50">Defense Stats</td>
            <td 
              v-for="weapon in weapons" 
              :key="weapon.id"
              class="border border-gray-200 px-4 py-2"
            >
              <div v-if="weapon.defense_stats && weapon.defense_stats.length > 0" class="space-y-1">
                ...
                <div v-if="weapon.defense_stats.length > 5" class="text-xs text-gray-500">
