...
<template>
  ...
  >
    ...
    <div v-if="items.length === 0" class="text-center py-16">
      ...
      <div class="text-xs text-surface-400">You can compare up to 3 items at once</div>
    </div>
      ...
      <div class="grid gap-4" :style="{ gridTemplateColumns: `repeat(${items.length}, 1fr)` }">
        ...
        >
          ...
          <div class="text-center space-y-2">
            <div
              class="h-16 bg-surface-100 dark:bg-surface-800 rounded flex items-center justify-center mb-3"
            >
          ...
          <template #content>
            <div class="space-y-3">
              ...
              >
                ...
                <div v-for="item in items" :key="`type-${item.id}`" class="text-sm">
              ...
              >
                ...
                >
                  ...
                  <i
                    ...
                    :class="{
                      'pi pi-check text-green-500 text-xs': canMeetRequirement(
                        statId,
                        getItemRequirement(item, statId)!
                      ),
                      'pi pi-times text-red-500 text-xs': !canMeetRequirement(
                        statId,
                        getItemRequirement(item, statId)!
                      ),
                    }"
          ...
          <template #content>
            <div class="space-y-3">
              ...
              <div v-if="hasAttackData">
                <h5 class="text-xs font-medium text-surface-600 dark:text-surface-400 mb-2">
                  Attack
                </h5>
                <div class="space-y-2">
                  <div
...
const hasRequirementsComparison = computed(() => commonRequirements.value.length > 0);

const hasCombatStats = computed(() =>
  props.items.some((item) => item.attack_stats || item.defense_stats)
);

const hasAttackData = computed(() => props.items.some((item) => item.attack_stats));

const hasDefenseData = computed(() => props.items.some((item) => item.defense_stats));
...
const recommendations = computed((): Recommendation[] => {
  ...
  if (commonStats.value.length > 0) {
    const statTotals = props.items.map((item) => ({
      id: item.id,
      total: commonStats.value.reduce(
        (sum, statId) => sum + Math.abs(getItemStat(item, statId)),
        0
      ),
    }));
