          ...
          <div class="stats-row data-row">
            ...
            <div v-if="hasDamageRange" class="stat-group">
              <div class="damage-display">
                ...
                <div
                  v-if="damageRange.minDamage && damageRange.maxDamage"
            ...
            <div class="stat-group">
              ...
              <div v-if="nanoStats.castTime" class="stat-pair">
                ...
                <span class="stat-val val-cast">{{
                  formatCentisecondsToSeconds(nanoStats.castTime)
                }}</span>
              </div>
              <div v-if="nanoStats.rechargeTime" class="stat-pair">
                <span class="stat-name">Recharge</span>
                <span class="stat-val val-cast">{{
                  formatCentisecondsToSeconds(nanoStats.rechargeTime)
                }}</span>
              </div>
            ...
            <div class="stat-group">
              ...
              <div v-if="tickInfo" class="stat-pair" data-testid="ticks-row">
                ...
                <span class="stat-val val-ticks">{{
                  formatTicks(tickInfo.tickCount, tickInfo.tickInterval)
                }}</span>
              </div>
            ...
            <div class="stat-group">
              ...
              <div v-if="nanoStats.targetType" class="stat-pair">
                ...
                <span class="stat-val val-target">{{
                  formatTargetType(nanoStats.targetType)
                }}</span>
              </div>
              <div v-if="nanoStats.flags && nanoStats.flags.length > 0" class="stat-pair">
                <span class="stat-name">Flags</span>
                <span class="stat-val val-flags"
                  >{{ nanoStats.flags.slice(0, 2).join(', ')
                  }}{{ nanoStats.flags.length > 2 ? '...' : '' }}</span
                >
              </div>
            </div>
          </div>
          ...
          <div class="stats-row data-row">
            ...
            <div class="stat-group">
              <div v-if="skillRequirementsFormatted.length > 0" class="modifier-list">
                <div
                  v-for="skill in skillRequirementsFormatted"
                  :key="skill.stat"
                  class="stat-pair"
                >
...
const hasSkillModifiers = computed(() => {
  return (
    (props.skillRequirements && props.skillRequirements.length > 0) ||
...
const damageRange = computed(() => {
      ...
      if (spellData.spells && Array.isArray(spellData.spells)) {
        for (const spell of spellData.spells) {
          if (damageSpellIds.includes(spell.spell_id)) {
            ...
            const maxDamage = Math.abs(
              spell.spell_params?.MaxValue || spell.spell_params?.MaxAmount || 0
            );
            ...
            if (minDamage > 10 || maxDamage > 10) {
              return {
                ...
                format:
                  SPELL_FORMATS[spell.spell_id as keyof typeof SPELL_FORMATS] ||
                  'Unknown damage format',
              };
            }
          }
        }
  ...
  if (props.item.description) {
    const damageMatch = props.item.description.match(
      /(\d{1,3}(?:,\d{3})*)-(\d{1,3}(?:,\d{3})*)\s+points?\s+of\s+damage/i
    );
    if (damageMatch) {
...
/* Color coding */
.nano-stats-component .val-cost {
  color: #8b5cf6;
}
.nano-stats-component .val-ncu {
  color: #6366f1;
}
.nano-stats-component .val-delta {
  color: #3b82f6;
}
.nano-stats-component .val-cast {
  color: #f59e0b;
}
.nano-stats-component .val-range {
  color: #10b981;
}
.nano-stats-component .val-duration {
  color: #06b6d4;
}
.nano-stats-component .val-ticks {
  color: #e11d48;
  font-weight: 600;
}
.nano-stats-component .val-school {
  color: #7c3aed;
}
.nano-stats-component .val-strain {
  color: #ef4444;
}
...
.nano-stats-component .val-stack {
  color: #84cc16;
}
.nano-stats-component .val-target {
  color: #64748b;
}
.nano-stats-component .val-flags {
  color: #a78bfa;
}
.nano-stats-component .val-requirement {
  color: #ef4444;
  font-weight: 600;
}
.nano-stats-component .val-bonus {
  color: #10b981;
  font-weight: 600;
}
.nano-stats-component .val-attack {
  color: #dc2626;
  font-weight: 600;
}
.nano-stats-component .val-defense {
  color: #059669;
  font-weight: 600;
}
