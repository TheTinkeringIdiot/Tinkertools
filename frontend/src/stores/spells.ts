...
export const useSpellsStore = defineStore('spells', () => {
  ...
  const spellsById = computed(
    () => (ids: number[]) => ids.map((id) => spells.value.get(id)).filter(Boolean) as Spell[]
  );

  const spellsByTarget = computed(
    () => (target: number) => allSpells.value.filter((spell) => spell.target === target)
  );
