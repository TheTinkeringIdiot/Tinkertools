...
export const usePocketBossesStore = defineStore('pocketBosses', () => {
  ...
  const pocketBossesById = computed(
    () => (ids: number[]) => ids.map((id) => pocketBosses.value.get(id)).filter(Boolean) as Mob[]
  );

  const pocketBossesByLevel = computed(
    () => (minLevel: number, maxLevel: number) =>
      allPocketBosses.value
        .filter((boss) => boss.level !== null && boss.level >= minLevel && boss.level <= maxLevel)
        .sort((a, b) => (a.level ?? 0) - (b.level ?? 0))
  );

  const pocketBossesByPlayfield = computed(
    () => (playfield: string) =>
  ...
  const levelRanges = computed(() => {
    ...
    const levels = allPocketBosses.value
      .map((boss) => boss.level)
   ...
   */
  async function getBossWithDrops(
    bossId: number
  ): Promise<(Mob & { dropDetails: SymbiantItem[] }) | null> {
