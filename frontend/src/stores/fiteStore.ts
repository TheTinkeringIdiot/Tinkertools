...
export const useFiteStore = defineStore('fite', () => {
  ...
  const isLikelyWeapon = (item: any): boolean => {
    ...
    const weaponSkillIds = [100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 152] // Weapon skills + Martial Arts
    const attackStatIds = [2, 7, 30] // Common attack-related stat IDs (damage, attack rating, etc.)
    
    return item.stats.some((stat: any) => 
      weaponSkillIds.includes(stat.stat) || attackStatIds.includes(stat.stat)
    )
  }
  ...
  const saveWeaponsToStorage = (): void => {
    try {
      localStorage.setItem('tinkertools_weapons_cache', JSON.stringify({
        data: weapons.value,
