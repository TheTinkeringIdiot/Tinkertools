...
export function useFilters(options: UseFiltersOptions = {}) {
  ...
  function applyItemFilters(items: Item[]): Item[] {
    ...
    if (filters.item_class?.length) {
      filtered = filtered.filter(
        (item) => item.item_class !== undefined && filters.item_class!.includes(item.item_class)
      );
    }
    ...
    if (filters.checkCompatibility && profilesStore.activeProfile) {
      filtered = filtered.filter((item) =>
        checkItemCompatibility(item, profilesStore.activeProfile!)
      );
    }
