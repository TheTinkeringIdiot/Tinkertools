 ...
 */
export function analyzeItemUsability(
  item: Item,
  character: Character
): ItemUsabilityResult {
  const issues: string[] = [];
  ...
  if (professionEffectiveness < 100) {
    issues.push(`${getProfessionName(character.profession)} only ${professionEffectiveness}% effective with this item type`);
  }
...
export function calculateCombatEffectiveness(equipment: EquipmentSet): {
  ...
  const offensiveStats = [
    276, 278, 279, 280, 281, 282, // Damage modifiers
    100, 101, 102, 103, 104, 105, 106, 107, 108 // Weapon skills
  ];
  ...
  const defensiveStats = [
    ...
    152, 153, 154, 155 // Defensive skills
  ];
