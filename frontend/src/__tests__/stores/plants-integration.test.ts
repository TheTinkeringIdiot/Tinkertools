...
vi.mock('@/services/api-client', () => ({
  apiClient: {
    ...
    getSymbiant: vi.fn((id: number) => Promise.resolve({
      success: true,
      data: {
        id,
        aoid: 100 + id,
...
describe('Plants Store Unit Tests', () => {
  ...
  describe('Performance and Caching', () => {
    ...
    it('uses efficient filtering for large datasets', () => {
      ...
      // Simulate filtering
      const filtered = mockSymbiants.filter(s => 
        s.family === 'Even' && s.slot === 'head'
      );
    ...
    it('manages memory efficiently with large builds', () => {
      ...
      const slots = ['head', 'eye', 'ear', 'rarm', 'chest', 'larm', 'waist', 'rwrist', 'legs', 'lwrist', 'rfinger', 'feet', 'lfinger'];
      ...
      slots.forEach((slot, index) => {
        largeBuild.symbiants[slot] = {
          ...
          slot,
          statBonuses: [
            { statId: 'strength', statName: 'Strength', value: 10, type: 'bonus' }
          ]
        };
      });
