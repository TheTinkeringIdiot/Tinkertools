...
vi.mock('../../components/items/ItemInterpolationBar.vue', () => ({
  default: {
    ...
    template:
      '<div class="interpolation-bar" @item-update="$emit(\'item-update\', $event)" @error="$emit(\'error\', $event)"></div>',
...
describe('ItemDetail', () => {
  const mockItem: Item = {
    ...
    spell_data: [{ id: 1, event: 1, spells: [] }],
    actions: [{ id: 1, action: 6, item_id: 1, criteria: [] }],
  };
    ...
    actions: [
      {
        ...
        action: 6,
        item_id: 1,
        criteria: [{ id: 1, value1: 112, value2: 400, operator: 2 }],
      },
    ],
  };
