...
describe.skipIf(!BACKEND_AVAILABLE)('TinkerPocket Integration Tests', () => {
  ...
  it('filters and searches work with real data', async () => {
    ...
    const avgLevel = Math.floor(
      (pocketBossStore.levelRange.min + pocketBossStore.levelRange.max) / 2
    );
  ...
  it('handles real API errors gracefully', async () => {
    ...
    expect(typeof component.$options.methods?.handleError || typeof component.handleError).toBe(
      'undefined'
    ); // No explicit handler needed in current implementation
  });
