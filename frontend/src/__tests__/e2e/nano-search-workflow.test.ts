...
vi.mock('primevue/badge', () => ({
  default: { name: 'Badge', template: '<span class="badge">{{ value }}</span>', props: ['value', 'severity'] }
}));
  ...
  default: {
    ...
    methods: {
      isCompatible(nano: any) {
        ...
        return nano.castingRequirements?.every((req: any) => {
          if (req.type === 'skill') {
  ...
  default: {
    ...
    methods: {
      ...
      getRequirementStatus(req: any) {
        ...
        if (req.type === 'level') {
          return this.activeProfile.level >= req.value ? '✓' : `${this.activeProfile.level}/${req.value}`;
        }
        return '?';
      }
    }
...
describe('Nano Search Workflow', () => {
  ...
  it('performs basic text search', async () => {
    ...
    const healNanos = filteredNanos.filter((nano: any) => 
      nano.name.toLowerCase().includes('heal') || 
      nano.description?.toLowerCase().includes('heal')
    );
  ...
  it('combines text search with school filter', async () => {
    ...
    const filteredNanos = wrapper.vm.filteredNanos;
    const matchingNanos = filteredNanos.filter((nano: any) => 
