...
vi.mock('primevue/checkbox', () => ({
  default: {
    name: 'Checkbox',
    template:
      '<input type="checkbox" v-model="modelValue" :id="inputId" :value="value" @change="$emit(\'update:modelValue\', $event.target.checked)" />',
...
vi.mock('primevue/chip', () => ({
  default: {
    name: 'Chip',
    template:
      '<span @click="$emit(\'click\')" :class="[\'cursor-pointer\', $attrs.class]"><slot>{{ label }}</slot></span>',
...
vi.mock('primevue/multiselect', () => ({
  default: {
    name: 'MultiSelect',
    template:
      '<select multiple v-model="modelValue"><option v-for="option in options" :key="option" :value="option">{{ option }}</option></select>',
...
vi.mock('primevue/slider', () => ({
  default: {
    name: 'Slider',
    template:
      '<input type="range" v-model="modelValue" :min="min" :max="max" :step="step" @input="$emit(\'update:modelValue\', [$event.target.value, modelValue[1] || max])" />',
...
describe('NanoFilters', () => {
  ...
  it('displays advanced filters when toggled', async () => {
    ...
    if (advancedButton.exists() && advancedButton.text().includes('Advanced')) {
      ...
      expect(wrapper.text()).toContain('Nano Point Cost') ||
