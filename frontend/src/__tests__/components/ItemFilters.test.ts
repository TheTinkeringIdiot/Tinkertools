...
vi.mock('primevue/accordiontab', () => ({
  default: {
    name: 'AccordionTab',
    template:
      '<div class="p-accordion-tab"><template #header><slot name="header" /></template><slot /></div>',
    props: ['header'],
  },
}));
...
vi.mock('primevue/multiselect', () => ({
  default: {
    name: 'MultiSelect',
    template:
      '<select multiple v-model="modelValue"><option v-for="opt in options" :key="opt.value" :value="opt.value">{{ opt.label }}</option></select>',
    props: [
      'modelValue',
      'options',
      'optionLabel',
